...
# ==========================================
# 1.5 簡易パスワード認証保護 (20分以上セッション保持 & 自動復元機能付き)
# ==========================================
...
if not st.session_state["authenticated"]:
    ...
    try:
        ...
        elif hasattr(st, "secrets") and "PASSWORD" in st.secrets:
...
# 30秒周期のバックグラウンド Ping 通信によるセッション維持
components.html("""
<script>
...
def calculate_auto_metrics(df_item_all, df_loc_all, selected_site_id, selected_sheet_type, form_data_dict, df_rec_latest, target_rep_id):
    ...
    # 3. SVI (Sludge Volume Index) 計算 [mL/g] = SV30 * 10000 / MLSS
    sv30_items = df_item_all[(df_item_all['Sheet_Type'] == selected_sheet_type) & (df_item_all['Item_Name'] == 'SV30')]
    for _, sv_row in sv30_items.iterrows():
        l_id = sv_row['Loc_ID']
        l_site = df_loc_all[df_loc_all['Loc_ID'] == l_id].iloc[0]['Site_ID']
        if l_site == selected_site_id:
            sv30_v = _parse_val(sv_row['Item_ID'])
            mlss_row = df_item_all[(df_item_all['Loc_ID'] == l_id) & (df_item_all['Item_Name'].isin(['MLSS', 'MLSS(簡)']))]
            if not mlss_row.empty:
                mlss_v = _parse_val(mlss_row.iloc[0]['Item_ID'])
                if sv30_v is not None and sv30_v > 0 and mlss_v is not None and mlss_v > 0:
                    svi_val = round((sv30_v * 10000.0) / mlss_v, 1)
                    svi_item_row = df_item_all[(df_item_all['Loc_ID'] == l_id) & (df_item_all['Item_Name'] == 'SVI')]
                    if not svi_item_row.empty:
                        calc_updates[svi_item_row.iloc[0]['Item_ID']] = str(svi_val)

    # 4. BOD除去率 [%] = (原水BOD - 処理水BOD) / 原水BOD * 100 (処理水BOD < 5.0 の場合は 5.0 として計算し '以上' を付与)
    if selected_sheet_type == '計量証明':
        site_loc_ids = df_loc_all[df_loc_all['Site_ID'] == selected_site_id]['Loc_ID']
        raw_loc_id = df_loc_all[(df_loc_all['Site_ID'] == selected_site_id) & (df_loc_all['Loc_Name'] == '原水')].iloc[0]['Loc_ID']
        
        raw_bod_item = df_item_all[(df_item_all['Loc_ID'] == raw_loc_id) & (df_item_all['Sheet_Type'] == '計量証明') & (df_item_all['Item_Name'].isin(['BOD', 'BOD5', 'C-BOD']))]
        treated_bod_item = df_item_all[(df_item_all['Loc_ID'].isin(site_loc_ids)) & (df_item_all['Loc_ID'] != raw_loc_id) & (df_item_all['Sheet_Type'] == '計量証明') & (df_item_all['Item_Name'].isin(['BOD', 'BOD5']))]
        rem_item = df_item_all[(df_item_all['Loc_ID'].isin(site_loc_ids)) & (df_item_all['Sheet_Type'] == '計量証明') & (df_item_all['Item_Name'] == 'BOD除去率')]
        
        if not raw_bod_item.empty and not treated_bod_item.empty and not rem_item.empty:
            raw_v = _parse_val(raw_bod_item.iloc[0]['Item_ID'])
            
            tr_item_id = treated_bod_item.iloc[0]['Item_ID']
            tr_raw_val = None
            if tr_item_id in form_data_dict:
                tr_raw_val = str(form_data_dict[tr_item_id])
            else:
                match = df_rec_latest[(df_rec_latest['Report_ID'] == target_rep_id) & (df_rec_latest['Item_ID'] == tr_item_id)]
                if not match.empty:
                    tr_raw_val = str(match.iloc[0]['Value'])
                    
            if raw_v is not None and raw_v > 0 and tr_raw_val is not None:
                tr_str = tr_raw_val.strip()
                is_less_than = "未満" in tr_str or "<" in tr_str
                
                tr_num = _parse_val(tr_item_id)
                if tr_num is None or is_less_than or tr_num < 5.0:
                    tr_num = 5.0
                    is_less_than = True
                    
                rem_val = round(((raw_v - tr_num) / raw_v) * 100.0, 1)
                formatted_rem = f"{rem_val}%以上" if is_less_than else f"{rem_val}%"
                calc_updates[rem_item.iloc[0]['Item_ID']] = formatted_rem
...
with tab2:
    ...
    if g_site_locs.empty:
        ...
        if not selected_loc_ids:
            ...
            curr_start_str = st.session_state["g_start_d"].strftime("%Y/%m/%d") if isinstance(st.session_state["g_start_d"], (datetime.date, pd.Timestamp)) else all_date_strs[0]
            ...
            site_reps_filtered = df_rep[(df_rep["Site_ID"] == selected_site_id) & (df_rep["Sheet_Type"] == selected_sheet_type)].copy()
            ...
            for l_idx, loc_id in enumerate(selected_loc_ids):
                ...
                item_y1_row = df_item[(df_item["Loc_ID"] == loc_id) & (df_item["Sheet_Type"] == selected_sheet_type) & (df_item["Item_Name"] == selected_item_y1)]
                ...
                if selected_item_y2:
                    item_y2_row = df_item[(df_item["Loc_ID"] == loc_id) & (df_item["Sheet_Type"] == selected_sheet_type) & (df_item["Item_Name"] == selected_item_y2)]
...
# ------------------------------------------
# TAB 3: エクセル帳票ダウンロード (要件⑥ + 2段ヘッダープレビュー + 全項目AIコメントカード)
# ------------------------------------------
with tab3:
    ...
    if items_ordered.empty:
        ...
        # 🤖 AI水質診断 ＆ 傾向分析コメント機能 (全項目網羅・特筆項目ダイジェスト出力)
        def get_ai_analysis_comment(selected_site_id, selected_sheet_type, df_export, col_defs):
            ...
            def format_date_short(d_str):
                parts = str(d_str).split('/')
                if len(parts) == 3:
                    return f"{int(parts[1])}/{int(parts[2])}"
                return str(d_str)

            site_name = site_options[selected_site_id]
            salient_bullets = []
            
            for item_id, loc_name, item_name in col_defs:
                unit_str = ""
                item_rows = df_item[(df_item["Item_ID"] == item_id)]
                if not item_rows.empty:
                    u_val = item_rows.iloc[0].get("Unit_or_Options", "")
                    if pd.notna(u_val) and str(u_val).strip() not in ["nan", ""]:
                        unit_str = f" {u_val}"
                ...
                for _, r in df_export.iterrows():
                    raw_v = r.get(item_id, "")
                    if pd.notna(raw_v) and str(raw_v).strip() not in ["", "-", "nan"]:
                        v_clean = str(raw_v).replace(",", "").replace("%", "").replace("<", "").replace(">", "").replace("未満", "").strip()
                        try:
                            num_v = float(v_clean)
                            date_num_pairs.append((str(r["Date"]), num_v, str(raw_v)))
                        except ValueError:
                            pass
                
                if len(date_num_pairs) >= 2:
                    nums = [p[1] for p in date_num_pairs]
                    mean_v = np.mean(nums)
                    std_v = np.std(nums)
                    latest_d, l_v_num, latest_raw_v = date_num_pairs[-1]
                    
                    # ○/○〜○/○にかけての上昇・低下傾向の自動判定
                    trend_msg = None
                    if len(date_num_pairs) >= 3:
                        tail = date_num_pairs[-6:]
                        n_t = len(tail)
                        ...
                        for i in range(n_t - 1, 0, -1):
                            if tail[i][1] > tail[i-1][1]:
                                ...
                                break
                        if inc_c >= 3:
                            s_d = format_date_short(tail[n_t - inc_c][0])
                            e_d = format_date_short(tail[-1][0])
                            s_v = tail[n_t - inc_c][1]
                            e_v = tail[-1][1]
                            trend_msg = f"<b>{s_d}〜{e_d}</b> にかけて {inc_c} 回連続で上昇傾向（{s_v:g} ➔ {e_v:g}{unit_str}）"
                        else:
                            # 連続低下チェック
                            dec_c = 1
                            for i in range(n_t - 1, 0, -1):
                                if tail[i][1] < tail[i-1][1]:
                                    ...
                                    break
                            if dec_c >= 3:
                                s_d = format_date_short(tail[n_t - dec_c][0])
                                e_d = format_date_short(tail[-1][0])
                                s_v = tail[n_t - dec_c][1]
                                e_v = tail[-1][1]
                                trend_msg = f"<b>{s_d}〜{e_d}</b> にかけて {dec_c} 回連続で低下傾向（{s_v:g} ➔ {e_v:g}{unit_str}）"
                            elif n_t >= 4:
                                first_v = tail[0][1]
                                last_v = tail[-1][1]
                                if first_v > 0 and (last_v - first_v) / first_v >= 0.25:
                                    s_d = format_date_short(tail[0][0])
                                    e_d = format_date_short(tail[-1][0])
                                    trend_msg = f"<b>{s_d}〜{e_d}</b> にかけて上昇傾向（{first_v:g} ➔ {last_v:g}{unit_str}）"
                                elif first_v > 0 and (first_v - last_v) / first_v >= 0.25:
                                    s_d = format_date_short(tail[0][0])
                                    e_d = format_date_short(tail[-1][0])
                                    trend_msg = f"<b>{s_d}〜{e_d}</b> にかけて低下傾向（{first_v:g} ➔ {last_v:g}{unit_str}）"

                    # 1. SVI 判定
                    if item_name == "SVI":
                        tr_suffix = f"（{trend_msg}）" if trend_msg else ""
                        if l_v_num > 150:
