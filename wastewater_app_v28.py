    ...
    if g_site_locs.empty:
        ...
        if not selected_loc_ids:
            ...
            if period_preset == "全期間":
                ...
                calc_start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(months=3)).date())
                calc_end_d = max_d
            elif period_preset == "過去6ヶ月":
                calc_start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(months=6)).date())
                calc_end_d = max_d
            elif period_preset == "過去1年":
                calc_start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(years=1)).date())
                calc_end_d = max_d
            elif period_preset == "過去2年":
                calc_start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(years=2)).date())
                calc_end_d = max_d
            else:
                calc_start_d = min_d
                calc_end_d = max_d
            ...
            # 日付文字列アラインメント
            calc_start_dt = pd.to_datetime(calc_start_d)
            calc_end_dt = pd.to_datetime(calc_end_d)
            closest_s_str = min(all_date_strs, key=lambda x: abs((pd.to_datetime(x) - calc_start_dt).days))
            closest_e_str = min(all_date_strs, key=lambda x: abs((pd.to_datetime(x) - calc_end_dt).days))

            d_col1, d_col2 = st.columns(2)
            s_idx = all_date_strs.index(closest_s_str) if closest_s_str in all_date_strs else 0
            e_idx = all_date_strs.index(closest_e_str) if closest_e_str in all_date_strs else len(all_date_strs) - 1

            with d_col1:
                sel_start_str = st.selectbox("📅 開始点検日", options=all_date_strs, index=s_idx, key=f"sel_start_{period_preset}")
            with d_col2:
                sel_end_str = st.selectbox("📅 終了点検日", options=all_date_strs, index=e_idx, key=f"sel_end_{period_preset}")

            start_d = pd.to_datetime(sel_start_str).date()
            end_d = pd.to_datetime(sel_end_str).date()
