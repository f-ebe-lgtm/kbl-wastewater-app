...
# ==========================================
# 1.5 簡易パスワード認証保護 (20分以上セッション保持 ＆ Secrets安全アクセス)
# ==========================================
...
# ==========================================
# GitHub REST API 自動同期関数 (Secrets安全アクセス)
# ==========================================
...
def calculate_auto_metrics(df_item_all, df_loc_all, selected_site_id, selected_sheet_type, form_data_dict, df_rec_latest, target_rep_id):
    ...
    # 3. SVI Calculation: SV30 * 10000 / MLSS (各現場固定Item_ID)
    sv30_mlss_map = {
...
# ==========================================
# データベース探射・自動作成・堅牢ロード処理 (絶対パス依存・未検出時白画面の完全排除)
# ==========================================
def find_excel_db():
...
with tab2:
    ...
    if g_site_locs.empty:
        ...
        if not selected_graph_locs:
            ...
            # 期間絞り込みプリセット（全期間 / 過去3ヶ月 / 過去6ヶ月 / 過去1年 / 過去2年 / 個別指定）
            preset_opt = st.radio(
                "⏱️ 期間プリセット選択",
