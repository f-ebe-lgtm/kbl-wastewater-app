...
# URLクエリパラメータからの認証セッション復元 (1時間・60分間無操作保持)
...
if not st.session_state["authenticated"] and qp_time and qp_hash:
    try:
        ...
        # 3600秒 (60分間) 無操作でもログイン状態を自動復元
        if qp_hash == expected_hash and (now_ts - auth_ts) < 3600:
...
if not st.session_state["authenticated"]:
    ...
    # ログイン中は定期的 (5分毎) にタイムスタンプを更新しセッションを自動延長
    if qp_time:
        try:
...
# ブラウザ接続維持 (Keep-Alive) スクリプト (2分毎にバックグラウンド通信して切断防止)
components.html("""
<script>
...
def calculate_auto_metrics(df_item_all, df_loc_all, selected_site_id, selected_sheet_type, form_data_dict, df_rec_latest, target_rep_id):
    ...
    # 3. SVI Calculation: SV30 * 10000 / MLSS (SV30の直後・右隣に自動配置)
    sv30_mlss_map = {
    ...
    # 4. BOD除去率 Calculation: (原水BOD - 処理水BOD) / 原水BOD * 100 (処理水BODの直後・右隣に自動配置)
    bod_rem_map = {
        'S001': ('I0049', 'I0054', 'I0235'),
        'S002': ('I0092', 'I0110', 'I0236'),
        'S004': ('I0167', 'I0182', 'I0237')
    }
...
# ------------------------------------------
# 2. データ読み込み ＆ キャッシュ処理 (桁区切りカンマの純粋削除)
# ------------------------------------------
@st.cache_data(ttl=1)
...
with tab1:
    ...
    if selected_loc_id:
        ...
        if loc_items.empty:
            ...
            # スマホ用テンキー (inputmode="decimal") 自動適用 JavaScript (Form外部配置)
            components.html("""
            <script>
            ...
            with st.form("inspection_input_form"):
                ...
                # スマホ1列折りたたみ時順序 (A1 ➔ B1 ➔ C1 ➔ A2 ➔ B2 ➔ C2) 3項目ずつの横行分割生成
                    ...
                    for c_idx, (_, item) in enumerate(row_chunk):
                        ...
                        with col_target:
                            if vtype == "Enum":
                                ...
                                form_data[item_id] = st.text_input(label_str, value=str(default_val) if str(default_val) != "nan" else "", key=f"txt_{item_id}")
...
with tab2:
    ...
    if g_site_locs.empty:
        ...
        if not selected_graph_locs:
            ...
            elif period_preset == "過去3ヶ月":
                start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(months=3)).date())
                end_d = max_d
            elif period_preset == "過去6ヶ月":
                start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(months=6)).date())
                end_d = max_d
            elif period_preset == "過去1年":
                start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(years=1)).date())
                end_d = max_d
            elif period_preset == "過去2年":
                start_d = max(min_d, (pd.to_datetime(max_d) - pd.DateOffset(years=2)).date())
                end_d = max_d
            else:
                col_dt1, col_dt2 = st.columns(2)
                with col_dt1:
                    start_d = st.date_input("開始点検日", value=min_d, min_value=min_d, max_value=max_d)
                with col_dt2:
                    end_d = st.date_input("終了点検日", value=max_d, min_value=min_d, max_value=max_d)

            st.info(f"💡 **選択中期間 ({start_d.strftime('%Y/%m/%d')} 〜 {end_d.strftime('%Y/%m/%d')})**")

            site_reps_filtered = site_reps.copy()
            site_reps_filtered["Date_dt"] = pd.to_datetime(site_reps_filtered["Date"]).dt.date
            site_reps_filtered = site_reps_filtered[(site_reps_filtered["Date_dt"] >= start_d) & (site_reps_filtered["Date_dt"] <= end_d)].sort_values("Date_dt")
                        ...
                        if not valid_y2.empty:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    mode="lines+markers",
                                    connectgaps=False,
                                    line=dict(color=color_cfg["y2"], width=2.5, dash="solid"),
            ...
            if not has_data:
                st.warning("選択された期間・槽・項目に該当する数値データが存在しません。")
            else:
...
# ------------------------------------------
# TAB 3: エクセル帳票ダウンロード (全データ選択 + AI水質診断カード)
# ------------------------------------------
with tab3:
    ...
    if items_ordered.empty:
        ...
        st.download_button(
            label=f"📥 【{file_year_str}】 {site_options[selected_site_id]} エクセル帳票をダウンロード",
            data=excel_data,
        ...
        # ------------------------------------------
        # 全項目対象 AI水質診断 ＆ 傾向分析コメントカード
        # ------------------------------------------
        st.markdown("---")
        st.markdown("<div class='sub-header'>🤖 全項目対象 AI水質診断 ＆ 傾向分析ダイジェスト</div>", unsafe_allow_html=True)

        latest_rep = reports.iloc[-1]
        latest_date_str = latest_rep["Date"]
        latest_notes = str(latest_rep["Notes"]) if pd.notna(latest_rep["Notes"]) else ""

        recs_site = df_rec[df_rec["Report_ID"].isin(reports["Report_ID"])].merge(
            df_item[["Item_ID", "Loc_ID", "Item_Name"]], on="Item_ID"
        ).merge(df_loc[["Loc_ID", "Loc_Name"]], on="Loc_ID")

        anomalies_summary = []
        good_summary = []

        for (loc_name, item_name), group in recs_site.groupby(["Loc_Name", "Item_Name"]):
            valid_vals = group.dropna(subset=["Value_Num"])["Value_Num"].values
            if len(valid_vals) >= 3:
                last_val = valid_vals[-1]
                hist_mean = np.mean(valid_vals[:-1]) if len(valid_vals) > 1 else np.mean(valid_vals)
                hist_std = np.std(valid_vals[:-1]) if len(valid_vals) > 1 else np.std(valid_vals)

                if hist_std > 0 and abs(last_val - hist_mean) > 2.0 * hist_std:
                    diff_ratio = ((last_val - hist_mean) / hist_mean) * 100.0 if hist_mean != 0 else 0
                    sign = "+" if diff_ratio > 0 else ""
                    anomalies_summary.append(f"・**【{loc_name}】{item_name}**: 最新値 `{last_val}` (過去平均: `{hist_mean:.1f}`, 乖離: `{sign}{diff_ratio:.1f}%`)")
                elif "除去率" in item_name and last_val >= 95.0:
                    good_summary.append(f"・**【{loc_name}】{item_name}**: 高水準維持 `{last_val}%` (安定処理)")
                elif "SVI" in item_name and last_val > 150:
                    anomalies_summary.append(f"・**【{loc_name}】{item_name}**: 膨化(バルキング)警戒値 `{last_val}` (基準150超過)")

        with st.container():
            st.markdown(f"""
            <div style="background-color: #F1F8E9; border-radius: 8px; padding: 16px; border-left: 6px solid #43A047;">
                <h4 style="color: #2E7D32; margin-top: 0;">📊 最新点検日: {latest_date_str} のAI自動水質診断レポート</h4>
            """, unsafe_allow_html=True)

            if latest_notes and latest_notes not in ["-", "nan", "None"]:
                st.markdown(f"📝 **現場特記事項メモ**: `{latest_notes}`")

            if anomalies_summary:
                st.markdown("⚠️ **【要注意・変動検知項目 (全項目照合)】**")
                for a_item in anomalies_summary[:4]:
                    st.markdown(a_item)
            else:
                st.markdown("✅ **全測定項目が過去の通常変動範囲内で推移しており、処理プロセスは極めて良好です。**")

            if good_summary:
                st.markdown("🌟 **【良好維持項目】**")
                for g_item in good_summary[:3]:
                    st.markdown(g_item)
