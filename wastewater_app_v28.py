...
def calculate_auto_metrics(df_item_all, df_loc_all, selected_site_id, selected_sheet_type, form_data_dict, df_rec_latest, target_rep_id):
    ...
    def _parse_val(item_id):
        if item_id in form_data_dict and form_data_dict[item_id] is not None:
            v_val = form_data_dict[item_id]
            if isinstance(v_val, (int, float)):
                return float(v_val)
            v_str = str(v_val).replace(',', '').replace('%', '').replace('<', '').replace('>', '').strip()
            try:
                return float(v_str)
            except ValueError:
                pass
    ...
    # Flexible item finder for unit-annotated item names
                ...
                for idx, (_, item) in enumerate(input_items.iterrows()):
                    ...
                    with col_target:
                        if vtype == "Enum":
                            ...
                            # Use st.number_input for numeric fields so mobile browsers natively trigger numeric keypad (テンキー)
                            default_num = None
                            if default_val is not None and str(default_val).strip() not in ["", "-", "nan"]:
                                try:
                                    default_num = float(str(default_val).replace(",", "").replace("%", ""))
                                except ValueError:
                                    default_num = None
                                    
                            form_data[item_id] = st.number_input(label_str, value=default_num, step=None, key=f"num_{item_id}")
